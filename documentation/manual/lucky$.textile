...
Lucky$ (aliased to $) is a DOM helper for use inside the core Lucky - but you can use it too if you like! Lucky$ provides some simple but useful functions for DOM manipulation and AJAX. 

If you have your favorite Ajax library that you just can't live without, then fret not - Lucky$ can live happily with which ever library you are most comfortable with.
...
The result returned will depend on the selector string. If the selector string is an ID (for example: #myDiv, ul#myElement) then a single DOM node will be returned (via the querySelector method). If the selector string is not an ID then an array will be returned containing 0 or more matching elements (via the querySelectorAll method)
...
Lucky$ provides a series of useful methods for dealing with DOM element class names
the Lucky$ class methods accept an element or a selector string as the first argument.

*hasClass*
@$.hasClass(element, className)@ will test if the element you pass in contains the class name. It only accept a single class as value to test - if you pass in multiple classes, seperated by a space then it will work if the element's className matches. But be aware - the browser does not gaurante the order of class names, so you should stick to testing a single class at a time.
...
Lucky provides a helper for making AJAX requests. There are two ways to call the function:
