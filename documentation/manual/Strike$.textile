...
The result returned will depend on the selector. If the selector is a string containing a DOM ID (for example: #myDiv, ul#myElement) then a single DOM node will be returned (via the querySelector method). If the selector string is not an ID then an array will be returned containing 0 or more matching elements (via the querySelectorAll method)
